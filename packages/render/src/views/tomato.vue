<template>
  <div id="Tomato">
    <van-count-down ref="countDown" :time="tomatoSec"/>
  </div>
...
onMounted(() => {
  ipcRenderer.on("tomato", (event, eventName, totalSec) => {
    if (eventName === "start") {
      tomatoSec.value = totalSec * 1000;
...
onUnmounted(() => {
  ipcRenderer.removeAllListeners("tomato");
})
</script>
